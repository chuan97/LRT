...
cm = ax.pcolormesh(
    wxs, ws, -Dm.T.imag, cmap="BuPu", norm=mpl.colors.LogNorm(vmin=vmin, vmax=vmax)
)
...
ax.set_title(
    rf"$\lambda/\Omega = {lam} \,,\; \omega_z/\Omega = {wz} \,,\; J / \Omega = {J / W}$",
    fontsize=14,
)
...
cm2 = axin2.pcolormesh(
    wxs, ws, chizzs.T.imag, cmap="OrRd", norm=mpl.colors.LogNorm(vmin=vminz, vmax=vmaxz)
)
axin2.text(
    0.5,
    0.8,
...
fig.savefig(
    "plots/alt_transverse_LMG_photon_response_letter.jpeg", bbox_inches="tight", dpi=300
)
# plt.show()
