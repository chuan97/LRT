...
cm = ax.pcolormesh(lam0s, ws, -Ds.T.imag, cmap="BuPu", norm=mpl.colors.LogNorm())
...
for i, N in enumerate(Ns):
    ...
    ax.plot(
        lam0s, N * (data["e2"] - data["e1"]), c="r", ls="--", alpha=(i + 1) / len(Ns)
    )
ax.set_ylim(0, ws_upperlimit)
ax.set_xlabel(r"$\lambda / \Omega$")
# ax.set_xticklabels([])
...
fig.savefig("plots/exact_diag_ising.jpeg", bbox_inches="tight", dpi=300)
