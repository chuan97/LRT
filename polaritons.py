...
def super_LMG(wz, wc, lam, J):
    ...
    B = np.sqrt(((wz/mu)**2 - 4*mu*J*wz - wc**2)**2 + 16*lam**2*mu*wc*wz)
...
def LMG(wz, wc, lam, J):
    if wz > 4*(lam**2/wc + J):
        # paramagnetic phase
        return normal(np.sqrt(wz*(wz - 4*J)), wc, lam*(1 - 4*J/wz)**(-1/4))
    else:
        # ferromagnetic phase
...
def f_a(wz, wc, lam, J):
    if wz:
        ...
        return wz*(1 - mu)*(3 + mu)/(8*mu*(1 + mu)) - J*(1 - mu)*(1 + 3*mu)/(2*(1 + mu))
    else:
