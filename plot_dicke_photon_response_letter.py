...
for i, lam in enumerate(lam0s):
    if z == 0:
        ...
        pm, pp = polaritons.dicke(
            np.sqrt(wz * (wz + 4 * lam**2 / (W * wz))),
            W,
...
ax.plot(lam0s, up_twoosc, c="gold", label=r"$\Omega_\pm$ without $P^2$ term", ls="--")
...
ax.plot(lam0s, up_twoosc, c="limegreen", label=r"$\Omega_\pm$ with $P^2$ term", ls="--")
...
fig.savefig("plots/dicke_photon_response_letter.jpeg", bbox_inches="tight", dpi=300)
